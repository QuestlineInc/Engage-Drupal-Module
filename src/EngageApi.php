...
class EngageApi {
  ...
  public function __construct() {
    $configFactory = \Drupal::config("questline_engage.settings");
    $this->api_base_url = $configFactory->get('api_url');
    $this->api_key = $configFactory->get("api_key");
  }
  ...
  public function getArticleEmbed($article_id = '', $article_type = '') {
    ...
    if ($article_id != '' && $article_type != '') {
      $url = $this->api_base_url . '/content/articles/' . $article_type . '/' . $article_id . '?expand=embed';
  ...
  public function getArticlePreview($article_id = '', $article_type = '') {
    ...
    $article = null;
    if ($article_id != '' && $article_type != '') {
      $url = $this->api_base_url . '/content/articles/' . $article_type . '/' . $article_id . '?format=html';
      $api_result = $this->makeApiGetCall($url);
      if (!$api_result['Error']) {
        if ($api_result['Info']['http_code'] == 200) {
          $decoded = json_decode($api_result['Response']);
  ...
  public function searchArticles($keyword = '', $page_index = 0, $page_size = 10) {
    ...
    $results = null;
    $url = $this->api_base_url . '/content/articles?search=' . urlencode($keyword) . '&page=index~' . $page_index . ',size~' . $page_size;
    $api_result = $this->makeApiGetCall($url);
    if (!$api_result['Error']) {
      if ($api_result['Info']['http_code'] == 200) {
        $decoded = json_decode($api_result['Response']);
